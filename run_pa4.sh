...
awk -F',' 'NR==1{next} {year=$(NF-1); dance=$10; if(year~/^[0-9]{4}$/ && dance+0>0 && dance+0<=1.0) {sum[year]+=dance; count[year]++}} END{for(y in sum) printf "%s\t%d\t%.3f\n", y, count[y], sum[y]/count[y]}' "$INPUT" | sort -k1,1n > out/year_danceability.tsv
